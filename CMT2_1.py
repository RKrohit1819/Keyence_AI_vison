...
plc.connect(
    "192.168.1.11", 0, 1
)  # IP address, rack, slot i.e 192.168.1.12 is IP address
    ...
    while True:
        ...
        if bool_to_read:
            ...
            if data[:4] == ["B", "2", "0", "5"]:
                ...
                # else:
                #     writeBool(8, 0, 4, True)
                #     time.sleep(1)
                #     writeBool(8, 0, 4, False)
                #     # unique.clear()
            else:
                data = read_char_list(7, 3, 27)
                # print("Character List:", data)
                # writeBool(8, 0, 3, False)
